...
#define MEM_CODE					0x02
#define MEM_XDATA					0x00
#define MEMTYPE_CODE					0x02
#define CMD_SPI_FLASH_EXTD_READ				0x65

#define CMD_SPI_ERASE_SPI_4KBBLOCK			0x63
...
BOOL AthensReadSPICodeSPIROM(int hub_index,BYTE *SPIUploadData, DWORD dstart_address, DWORD dend_address_ul)
{
	...
	int bRetVal = FALSE;
	//USB57x4
	DWORD dwToReadFromCodeSpace,dwToExtdRead, dwExtReadStartAddr;
	WORD wLength;
	float fProgressPos =0;
	float fProgressAlloc;
	USB_CTL_PKT UsbCtlPkt;
	do
	{
		...
		/*******************************USB57x4***************************************************************/
		if(dstart_address < (DWORD) 0x10000)
		{
			dwToReadFromCodeSpace = ((((DWORD) 0xFFFF - dstart_address +1) < (dend_address_ul - dstart_address + 1)) ? ((DWORD) 0xFFFF - dstart_address +1) : (dend_address_ul - dstart_address + 1));
			dwToExtdRead = (dend_address_ul - dstart_address + 1) - dwToReadFromCodeSpace;
			dwExtReadStartAddr = (dstart_address + dwToReadFromCodeSpace);
		}
		else
		{
			dwToReadFromCodeSpace = 0;
			dwToExtdRead	= dend_address_ul - dstart_address + 1;
			dwExtReadStartAddr = dstart_address;
		}
		if(dwToReadFromCodeSpace)
		{
			fProgressAlloc = ((float) 100 * dwToReadFromCodeSpace) / (dwToReadFromCodeSpace + dwToExtdRead);
			fProgressAlloc /= ((float) dwToReadFromCodeSpace / SPI_READ_BUF_SIZE);
			
			//call set mem type to set code space
			bRetVal = libusb_control_transfer((libusb_device_handle*)gasHubInfo[hub_index].handle, 0x41,CMD_SET_MEM_TYPE, MEM_CODE, 
								0, 0,0, CTRL_TIMEOUT);

			i = dstart_address;
			while (dwToReadFromCodeSpace)
			{
				wLength = (WORD )((dwToReadFromCodeSpace < SPI_READ_BUF_SIZE) ? dwToReadFromCodeSpace : SPI_READ_BUF_SIZE);

				memset (abyRecBuf, 0xCC, wLength);

				bRetVal = libusb_control_transfer((libusb_device_handle*)gasHubInfo[hub_index].handle, 0x41,CMD_SET_MEM_TYPE, MEMTYPE_CODE, 
								0, 0,0, CTRL_TIMEOUT);
				if(bRetVal < 0)
				{
					break;
				...
				if(FALSE == USB2530_Device_Read_Code_Memory (hub_index, LOWORD(i) , &abyRecBuf[0], wLength, NULL))
				{
					DEBUGPRINT("ReadSPIFlash : Failed, Device_Read_Code_Memory");
					break;
				}
				memcpy(&SPIUploadData[i - dstart_address], &abyRecBuf[0], wLength);

				i += wLength;
				dwToReadFromCodeSpace -= wLength;

				fProgressPos += fProgressAlloc;
			}
			bRetVal = libusb_control_transfer((libusb_device_handle*)gasHubInfo[hub_index].handle, 0x41,CMD_SET_MEM_TYPE,MEM_XDATA, 
								0, 0,0, CTRL_TIMEOUT);
		}
		if (dwToExtdRead)
		{
			fProgressAlloc = ((float) 100 * dwToExtdRead) / (dwToReadFromCodeSpace + dwToExtdRead);
			fProgressAlloc /= ((float) dwToExtdRead / SPI_READ_BUF_SIZE);

			i = dwExtReadStartAddr;

			while (dwToExtdRead)
			{
				wLength = (WORD)min (dwToExtdRead,SPI_READ_BUF_SIZE);

				memset (abyRecBuf, 0xCC, wLength);
				//SPI_FlashExtdRead
				bRetVal = libusb_control_transfer((libusb_device_handle*)gasHubInfo[hub_index].handle, 0xc1, CMD_SPI_FLASH_EXTD_READ,LOWORD(i),
						0,&abyRecBuf[0],wLength, CTRL_TIMEOUT);
				if(bRetVal < 0)
				{
					DEBUGPRINT("ReadSPIFlash : Failed, Device_Read_Code_Memory");
					break;
			...
			else
			{
				for(i = dstart_address; TRUE == bCheck_flag; i+= (MAX_DOWNLOAD_SIZE))
				{
					...
					if(bCheck_flag)
					{
						...
						{
							rc = libusb_control_transfer((libusb_device_handle*)gasHubInfo[DevID].handle,0x41,CMD_SPI_BYTEFLASH_PGM,
                                (uint16_t)gpSPICmdList [m].wTotalLength,0,
...
DWORD SPI_SignatureErase(unsigned int DevID,int iRestartDelay)
{
	...
	do
	{
		/****************************************Special handling for Tyler***********************************************/
		//Erase 4kb, 0x0F000 is Page address
		byBuffer[0] = HIBYTE(HIWORD(0x0F000));
		byBuffer[1] = LOBYTE(HIWORD(0x0F000));
		byBuffer[2] = HIBYTE(LOWORD(0x0F000));
		byBuffer[3] = LOBYTE(LOWORD(0x0F000));
	
		rc =libusb_control_transfer((libusb_device_handle*)gasHubInfo[DevID].handle,0x41,CMD_SPI_ERASE_SPI_4KBBLOCK,0,0,byBuffer,
				4,CTRL_TIMEOUT);
		if(rc < 0)
		{
			DEBUGPRINT("Failed to Erase SPI content \n");
			break;
		}
		//Reset the device.
...
BOOL USB2530_Device_Read_Code_Memory (/*[In]*/ int hub_index , 
				/*[In]*/ WORD wStartAddress , 
				/*[In]*/ unsigned char ucharReceivedBuffer[] , 
				/*[In]*/ WORD wBufLength,
				/*[Out]*/ DWORD *pdwActualLength)
{
	BOOL  CmdStatue = true;
	DWORD dwError = 0;
	int bRet = FALSE;
	...
	do
	{
		bRet = libusb_control_transfer((libusb_device_handle*)gasHubInfo[hub_index].handle, 0x41,CMD_SET_MEM_TYPE, MEMTYPE_CODE, 
								0, 0,0, CTRL_TIMEOUT);
		if(bRet < 0)
		{
			break;
		...
		//READ
		bRet = libusb_control_transfer((libusb_device_handle*)gasHubInfo[hub_index].handle,0xC1,0x04,wStartAddress,0,(BYTE*)ucharReceivedBuffer,
			wBufLength,CTRL_TIMEOUT);
		if(bRet < 0)
		{
			bRet = FALSE;
		}
